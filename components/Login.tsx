...
const Login = () => {
  ...
  async function submitHandler() {
    ...
    if(isLogIn){
      try{
        ...
        setError('Incorrect email and password or Register youreself')
      }
      return;
    }
  ...
  return (
    <div className="flex-1 flex flex-col justify-center items-center gap-4">
      ...
      <h2 className="duration-300 hover:scale-110 cursor-pointer border p-1 py-2 w-full max-w-[35ch] text-center" onClick={()=> setIsLogIn(!isLogIn)}>{!isLogIn ? 'Login' : 'Register'}</h2>
    </div>
  );
};
